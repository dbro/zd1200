...
sleds through zeros.  This script therefore:
  ...
  2. applies the six zd1200-patch.gdb byte patches to the ELF
     (virtual address -> file offset via the ELF32 PT_LOAD segments),
...
# Decoded from dbro_zd1200/zd1200-patch.gdb: (virtual_address, patch_bytes).
# These are fixed at kernel build time for this 2.6.32.24 image.
PATCHES = [
    (0xC10416F0, bytes.fromhex("c3")),                             # kernel_halt(): no appliance power controller
    (0xC145090F, bytes.fromhex("31c0c3")),                         # rks_pkt_trace_init(): skip tif0 path
    (0xC1015DB0, bytes.fromhex("c3")),                             # machine_restart(): QEMU supervises restart
    (0xC123C600, bytes.fromhex("31c0c3")),                         # COB7402 reset/watchdog function -> no-op
    (0xC123D06D, bytes.fromhex("e914ffffff")),                     # skip physical board-data retry/recovery path
    (0xC123BC20, bytes.fromhex("c7020252541266c74204000131c0c3")), # board-data query: MAC 02:52:54:12:00:01, success
]
...
def va_to_offset(data: bytes, va: int) -> int:
    """Map a kernel virtual address to a file offset via the ELF32 PT_LOADs."""
    if data[:4] != b"\x7fELF" or data[4] != 1:
        raise SystemExit("payload is not a 32-bit ELF")
    e_phoff = struct.unpack_from("<I", data, 28)[0]
    e_phentsize = struct.unpack_from("<H", data, 42)[0]
    e_phnum = struct.unpack_from("<H", data, 44)[0]
    for i in range(e_phnum):
        off = e_phoff + i * e_phentsize
        p_type, p_offset = struct.unpack_from("<II", data, off)
    ...
    raise SystemExit(f"virtual address 0x{va:08x} is not in any PT_LOAD segment")
...
def main():
    ...
    ap.add_argument("--vmlinux", default="image/vmlinux",
                    help="pristine kernel ELF for a cross-check (default image/vmlinux)")
    args = ap.parse_args()
    ...
    # Cross-check against the pristine vmlinux produced by prepare-vendor-image.sh.
    vmlinux = Path(args.vmlinux)
    if vmlinux.exists():
        want = vmlinux.read_bytes()
        if want != payload:
            raise SystemExit("member payload does not match image/vmlinux; refusing to patch")
        print(f"payload matches {args.vmlinux}")

    # Apply the six byte patches.
    elf = bytearray(payload)
    for va, patch in PATCHES:
        fo = va_to_offset(elf, va)
        original = bytes(elf[fo:fo + len(patch)])
        if original == patch:
            print(f"  0x{va:08x}: already patched (offset 0x{fo:x})")
        else:
            print(f"  0x{va:08x}: {original.hex()} -> {patch.hex()} (offset 0x{fo:x})")
